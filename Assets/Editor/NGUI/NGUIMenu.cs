...
static public class NGUIMenu
{
	...
	static public GameObject SelectedRoot ()
	{
		...
		if (go == null)
		{
			Mogo.Util.LoggerHelper.Debug("No UI found. You can create a new one easily by using the UI creation wizard.\nOpening it for your convenience.");
			CreateUIWizard();
		}
		return go;
	}
	...
	static public void AddSprite ()
	{
		...
		{
			Mogo.Util.LoggerHelper.Debug("You must select a game object first.");
		}
	}
	...
	static public void AddCollider ()
	{
		...
		if (NGUIEditorTools.WillLosePrefab(go))
		{
			...
			{
				Mogo.Util.LoggerHelper.Debug("You must select a game object first, such as your button.");
			}
		}
	}
	...
	static void PixelPerfectSelection ()
	{
		...
		{
			Mogo.Util.LoggerHelper.Debug("You must select an object in the scene hierarchy first");
			return;
		}
