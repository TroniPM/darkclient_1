...
public class RuneUIPackageGrid : MonoBehaviour
{
    ...
    void OnDrag(Vector2 v)
    {
        ...
        if (!m_bIsDragBegin)
        {
            EventDispatcher.TriggerEvent<int>("RuneUIDragBegin", ID);
            m_bIsDragBegin = true;
        }
    ...
    void OnPress(bool isOver)
    {
        if (isOver)
        {
            ...
            m_iDragStartID = m_rayHit.transform.GetComponentsInChildren<RuneUIPackageGrid>(true)[0].ID;

            EventDispatcher.TriggerEvent<int>("RuneUIPackageGridDown", ID);
        }
        else
        {
            if (m_bStartCount)
            ...
            if (m_bIsDragging)
            {
                ...
                {
                    if (m_rayHit.transform.GetComponentsInChildren<RuneUIPackageGrid>(true).Length > 0)
                    {
                        int id = m_rayHit.transform.GetComponentsInChildren<RuneUIPackageGrid>(true)[0].ID;

                        EventDispatcher.TriggerEvent<int, int>("RuneUIPackageGridDrag", id, m_iDragStartID);

                        //m_iDragStartID = id;
                    }
                    else if (m_rayHit.transform.GetComponentsInChildren<RuneUIInsetGrid>(true).Length > 0)
                    {
                        int id = m_rayHit.transform.GetComponentsInChildren<RuneUIInsetGrid>(true)[0].ID;

                        EventDispatcher.TriggerEvent<int, int>("RuneUIPackageGridDragToInsetGrid",id,m_iDragStartID);
                    }
                }
                else
                {
                    EventDispatcher.TriggerEvent("RuneUIPackageDragOutside");
                }
            }
