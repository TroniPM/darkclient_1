...
public class MeleeWeaponTrail : MonoBehaviour
{
    ...
    public Material Material { set { _material = value; if (_trailObject)_trailObject.GetComponent<Renderer>().material = _material; } }
	...
	void Start()
	{
		...
		_trailObject.AddComponent<MeshFilter>();
        ...
        DontDestroyOnLoad(_trailObject);
    }
			...
			if (theDistanceSqr > _minVertexDistanceSqr)
			{
				...
				if (make)
				{
					...
					if (_points.Count >= 4)
					{
						...
						//LoggerHelper.Debug(" smoothTipList.Count: " + smoothTipList.Count);
