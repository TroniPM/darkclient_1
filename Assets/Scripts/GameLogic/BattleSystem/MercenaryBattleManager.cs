...
namespace Mogo.Game
{
    public class MercenaryBattleManager : BattleManager
    {
        ...
        override public void OnHit(int _spellID, uint _attackerID, uint woundId, List<int> harm)
        {
            ...
            if (MogoWorld.showHitShader && (theOwner as EntityMercenary).HitShader != null)
            {
                int _h = Utils.Choice<int>((theOwner as EntityMercenary).HitShader);
                if (_h == 1)
                {//等于1闪白
            ...
            if (MogoWorld.showFloatBlood && harm.Count <= 3 && SkillAction.dataMap[_spellID].damageFlag == 1)
            {
        ...
        {
            LoggerHelper.Debug("MercenaryBattleManager OnDead");
